...
******************************************************************************
*/
#define ST25R3911_SS_PIN            GPIO_PIN_4         /*!< GPIO pin used for ST25R3911 SPI SS                */ 
#define ST25R3911_SS_PORT           GPIOA   /*!< GPIO port used for ST25R3911 SPI SS port          */ 

#define ST25R3911_INT_PIN           GPIO_PIN_3        /*!< GPIO pin used for ST25R3911 External Interrupt    */
#define ST25R3911_INT_PORT          GPIOA  /*!< GPIO port used for ST25R3911 External Interrupt   */
...
//#define timerCalculateTimer(t)   (HAL_GetTick() +t)

extern uint8_t SpiTxRx(const uint8_t *txData, uint8_t *rxData, uint8_t length);
extern uint8_t timerIsExpired( uint32_t timer );
extern uint32_t HAL_GetTick(void);
extern uint32_t timerCalculateTimer( uint16_t time );
extern void 	antennaAmplitudeCalibrate(void);
/*
******************************************************************************
* GLOBAL MACROS
******************************************************************************
*/
#define platformIrqST25R3911Disable()                 HAL_NVIC_DisableIRQ(EXTI3_IRQn)             /*!< Disables ST353911 MCU's external interrupt  */
#define platformIrqST25R3911Enable()                  HAL_NVIC_EnableIRQ(EXTI3_IRQn)              /*!< Enables ST353911 MCU's external interrupt   */
...
#define platformGpioSet( port, pin )              		(port->BSRR = pin) //HAL_GPIO_WritePin(port, pin, GPIO_PIN_SET)  /*!< Turns the given GPIO High                   */
#define platformGpioClear( port, pin )                ( port->BSRR = (uint32_t)pin << 16u)// HAL_GPIO_WritePin(port, pin, GPIO_PIN_RESET)  /*!< Turns the given GPIO Low                    */

//#define platformGpioSet( port, pin )              	    ( port->BSRR = (uint32_t)pin << 16u)	//HAL_GPIO_WritePin(port, pin, GPIO_PIN_SET)  /*!< Turns the given GPIO High                   */
//#define platformGpioClear( port, pin )            (port->BSRR = pin) // HAL_GPIO_WritePin(port, pin, GPIO_PIN_RESET)  /*!< Turns the given GPIO Low                    */


#define platformGpioToogle( port, pin )               //HAL_GPIO_TogglePin(port, pin)                 /*!< Toogles the given GPIO                      */
#define platformGpioIsHigh( port, pin )               (HAL_GPIO_ReadPin(port, pin) == 1) /*!< Checks if the given LED is High             */
